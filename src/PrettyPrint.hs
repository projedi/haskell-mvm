...
prettyPrintExpr n (ExprPlus el er) = parenIfNeeded n 1 $ prettyPrintExpr 1 el ++ " + " ++ prettyPrintExpr 1 er
prettyPrintExpr n (ExprMinus el er) = parenIfNeeded n 1 $ prettyPrintExpr 1 el ++ " - " ++ prettyPrintExpr 1 er
prettyPrintExpr n (ExprTimes el er) = parenIfNeeded n 2 $ prettyPrintExpr 2 el ++ " * " ++ prettyPrintExpr 2 er
prettyPrintExpr n (ExprDiv el er) = parenIfNeeded n 2 $ prettyPrintExpr 2 el ++ " / " ++ prettyPrintExpr 2 er
prettyPrintExpr n (ExprMod el er) = parenIfNeeded n 2 $ prettyPrintExpr 2 el ++ " % " ++ prettyPrintExpr 2 er
prettyPrintExpr n (ExprEq el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " == " ++ prettyPrintExpr 0 er
prettyPrintExpr n (ExprNeq el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " != " ++ prettyPrintExpr 0 er
prettyPrintExpr n (ExprLt el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " < " ++ prettyPrintExpr 0 er
prettyPrintExpr n (ExprLeq el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " <= " ++ prettyPrintExpr 0 er
prettyPrintExpr n (ExprGt el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " > " ++ prettyPrintExpr 0 er
prettyPrintExpr n (ExprGeq el er) = parenIfNeeded n 0 $ prettyPrintExpr 0 el ++ " >= " ++ prettyPrintExpr 0 er
...
instance PrettyPrintSimple FunctionCall where
  prettyPrintSimple (FunctionCall funname args) = prettyPrintSimple funname ++ paren (List.intercalate ", " (map (prettyPrintExpr 0) args))

instance PrettyPrintSimple FunctionDecl where
  prettyPrintSimple (FunctionDecl retType funname params) = prettyPrintSimple retType ++ " " ++ prettyPrintSimple funname ++ paren (List.intercalate ", " (map prettyPrintSimple params))

instance PrettyPrintSimple VarDecl where
  prettyPrintSimple (VarDecl vtype name) = prettyPrintSimple vtype ++ " " ++ prettyPrintSimple name
...
prettyPrintStatement n (StatementWhile e s) = indent n ("while (" ++ prettyPrintExpr 0 e ++ ")") ++ hangingStatement n s
prettyPrintStatement n (StatementIf e s) = indent n ("if (" ++ prettyPrintExpr 0 e ++ ")") ++ hangingStatement n s
prettyPrintStatement n (StatementFor v e1 e2 s) = indent n ("for (" ++ prettyPrintSimple v ++ " in " ++ prettyPrintExpr 10 e1 ++ ".." ++ prettyPrintExpr 10 e2 ++ ")") ++ hangingStatement n s
...
prettyPrintStatement n (StatementIfElse e s1@(StatementBlock _) s2) = indent n ("if (" ++ prettyPrintExpr 0 e ++ ")") ++ hangingStatement n s1 ++ " } else" ++ hangingStatement n s2
prettyPrintStatement n (StatementIfElse e s1 s2) = indent n ("if (" ++ prettyPrintExpr 0 e ++ ")") ++ hangingStatement n s1 ++ "\n" ++ indent n "else" ++ hangingStatement n s2
prettyPrintStatement n (StatementBlock stmts) = indent n "{\n" ++ printProgram (n + 1) stmts ++ "\n" ++ indent n "}"

printProgram :: Int -> [Statement] -> String
printProgram n stmts = List.intercalate "\n" (map (prettyPrintStatement n) stmts)
